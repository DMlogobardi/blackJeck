...
public class Blackjeck {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					Blackjeck window = new Blackjeck();
	 ...
	 */
	public Blackjeck() throws UnsupportedAudioFileException, IOException, LineUnavailableException {
		initialize();
	}
		...
		if (start == true) {
			if (stai == false) {
				...
				if (punti <= 21) {
					...
					ImageIcon carta = new ImageIcon(mazzo.get(usciti.get(3)).getFronte());
					banco.get(1).setIcon(carta);
					...
					while (pesca == true) {
						if (puntiBanco <= 16) {
							...
							ImageIcon carta1 = new ImageIcon(mazzo.get(indice).getFronte());
							banco.get(lableBanco).setIcon(carta1);
							puntiBanco += valore(indice, isBanco = true);
							lableBanco = lableBanco + 1;
						} else
							pesca = false;
					}
					fineMano();
	...
	private void initialize() throws UnsupportedAudioFileException, IOException, LineUnavailableException {
		...
		File file = new File("Music/gioco.wav");
